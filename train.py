...
class get_al_hyperparams:
    def __init__(self, dataset_name="voc"):
        ...
        self.dataset_path = {
            "voc": "/kaggle/working/AL-SSL/data/VOC0712",
...
parser.add_argument(
    "--dataset_name",
    default=al_hyperparams.get_dataset_name(),
    type=str,
    help="Dataset name",
)
parser.add_argument(
    "--cfg",
    default=al_hyperparams.get_config(),
    type=dict,
...
parser.add_argument(
    "--dataset",
    default="VOC300",
    choices=["VOC300", "VOC512"],
    type=str,
    help="VOC300 or VOC512",
)
parser.add_argument(
    "--dataset_root",
    default=al_hyperparams.get_dataset_path(),
    type=str,
    help="Dataset root directory path",
)
parser.add_argument(
    "--num_total_images",
    default=al_hyperparams.get_num_ims(),
    type=int,
    help="Number of images in the dataset",
)
parser.add_argument(
    "--num_initial_labeled_set",
    default=al_hyperparams.get_num_init(),
    type=int,
    help="Number of initially labeled images",
)
parser.add_argument(
    "--acquisition_budget", default=1000, type=int, help="Active labeling cycle budget"
)
parser.add_argument(
    "--num_cycles", default=5, type=int, help="Number of active learning cycles"
)
parser.add_argument(
    "--criterion_select",
    default="combined",
...
parser.add_argument("--id", default=2, type=int, help="the id of the experiment")
parser.add_argument(
    "--basenet", default="vgg16_reducedfc.pth", help="Pretrained base model"
)
parser.add_argument(
    "--batch_size",
    default=al_hyperparams.get_batch_size(),
    type=int,
...
parser.add_argument(
    "--save_folder",
    default="../al_ssl/weights/",
    help="Directory for saving checkpoint models",
)
parser.add_argument(
    "--net_name", default=None, type=str, help="the net checkpoint we need to load"
)
parser.add_argument(
    "--is_apex",
    default=0,
...
parser.add_argument(
    "--pseudo_threshold",
    default=al_hyperparams.get_pseudo_threshold(),
    type=float,
...
def compute_consistency_loss(
    conf, loc, conf_flip, loc_flip, conf_consistency_criterion
):
    ...
    conf_sampled_flip = conf_mask_sample_flip[mask_conf_index].view(
        -1, args.cfg["num_classes"]
    )
    ...
    if mask_val.sum() > 0:
        ...
        consistency_conf_loss_a = (
            conf_consistency_criterion(conf_sampled.log(), conf_sampled_flip.detach())
            .sum(-1)
            .mean()
        )
        consistency_conf_loss_b = (
            conf_consistency_criterion(conf_sampled_flip.log(), conf_sampled.detach())
            .sum(-1)
            .mean()
        )
        ...
        consistency_loc_loss_x = torch.mean(
            torch.pow(loc_sampled[:, 0] + loc_sampled_flip[:, 0], exponent=2)
        )
        consistency_loc_loss_y = torch.mean(
            torch.pow(loc_sampled[:, 1] - loc_sampled_flip[:, 1], exponent=2)
        )
        consistency_loc_loss_w = torch.mean(
            torch.pow(loc_sampled[:, 2] - loc_sampled_flip[:, 2], exponent=2)
        )
        consistency_loc_loss_h = torch.mean(
            torch.pow(loc_sampled[:, 3] - loc_sampled_flip[:, 3], exponent=2)
        )

        consistency_loc_loss = torch.div(
            consistency_loc_loss_x
            + consistency_loc_loss_y
            + consistency_loc_loss_w
            + consistency_loc_loss_h,
            4,
        )
...
def train(dataset, data_loader, cfg, labeled_set, supervised_dataset, indices):
    ...
    conf_consistency_criterion = torch.nn.KLDivLoss(
    ...
    while finish_flag:
        ...
        for iteration in range(cfg["max_iter"]):
            ...
            if len(sup_image_index) == 0:
                loss_l, loss_c = torch.cuda.FloatTensor([0]), torch.cuda.FloatTensor(
                    [0]
                )
            else:
                loss_l, loss_c = criterion(
                    output, targets, np.array(new_semis)[semis_index]
                )
            ...
            if iteration % 10 == 0:
                ...
                print(
                    "iter "
                    + repr(iteration)
                    + ": loss: %.4f , loss_c: %.4f , loss_l: %.4f , loss_con: %.4f, lr : %.4f, super_len : %d\n"
                    % (
                        loss.data,
                        loss_c.data,
                        loss_l.data,
                        consistency_loss.data,
                        float(optimizer.param_groups[0]["lr"]),
                        len(sup_image_index),
                    )
                )

            if iteration != 0 and (iteration + 1) % 2500 == 0:
    ...
    for i in range(args.num_cycles):
        ...
        if args.do_AL:
            if args.criterion_select in ["Max_aver", "entropy", "random"]:
                ...
                labeled_set, unlabeled_set = active_learning_entropy(
                    args,
                    batch_iterator,
                    labeled_set,
                    unlabeled_set,
                ...
                batch_iterator = iter(unsupervised_data_loader)
                labeled_set, unlabeled_set = active_learning_inconsistency(
                    args,
                    batch_iterator,
                    labeled_set,
                    unlabeled_set,
                ...
                print("Combined")
                batch_iterator = iter(unsupervised_data_loader)
                labeled_set, unlabeled_set = combined_score(
                    args,
                    batch_iterator,
                    labeled_set,
                    unlabeled_set,
